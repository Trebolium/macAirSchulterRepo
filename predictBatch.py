...
for f in files:
	...
	for current_Index in tqdm(range(half_window+1,int((max_index-1)*float(int(sys.argv[3])/100)))):
		...
		if windowed_feature.shape[2]!=115:
			pdb.set_trace()
		...
		for label_row,_ in enumerate(label_points):
			...
			if all_pred_list[pred_row][0]<label_points[label_row][0]:
				...
				if all_pred_list[pred_row][4]==label_points[label_row-1][2]:
					...
					# increment pred_row
					break
			if label_row==len(label_points)-1:
				# we can now assume that label entries have finished
				# therefore the last remaining preds should = the last label entry
				if all_pred_list[pred_row][4]==label_points[label_row][2]:
					# and label equals 1
					if label_points[label_row][2]==1:
						# both are 1!
						true_pos+=1
						performance_list_name.append('true_pos')
						performance_list_value.append(5)
	...
	with open('songPredictions/'+sys.argv[4]+'/'+os.path.basename(f)[:-4]+'.csv', "w") as csvFile:
		...
		header='Time','Raw_Pred','Rounded_Raw_Pred','Filtered_Pred','Round_Filtered_Performance_Name','Performance_Value'
	...
	if true_poss+false_poss>0:
		precision=true_poss/(true_poss+false_poss)
		print('precision: ', precision)
	else: print('No positives detected to calculate precision!')

	if true_poss+false_negs>0:
		recall=true_poss/(true_poss+false_negs)
		print('recall: ',recall)
	else: print('No positive labels to calculate recall!')

	if true_poss+false_negs>0:
		specifity=true_negs/(true_poss+false_negs)
		print('specifity: ',specifity)
	else: print('No positive labels to calculate specifity!')

	if precision+recall>0:
		f_score=2*((precision*recall)/(precision+recall))
		print('F Score: ', f_score)
	else: print('No precision or recall to calculate F Score')

	evaluation_list.append((label_name+': ',true_poss,true_negs,false_poss,false_negs,precision,recall,specifity,f_score))

with open('songPredictions/'+sys.argv[4]+'/'+'allSongsEvaluation.csv', "w") as csvFile:
	writer = csv.writer(csvFile)
	titles='Title','True_Poss','True_Negs','False_Poss','False_Negs','Precision','Recall','Specifity','F-Score'
	writer.writerow(titles)
	for row in evaluation_list:
		writer.writerow(row)
csvFile.close()
...
print('EVALUATION FOR ENTIRE BATCH')
print('true_poss: ',true_poss)
print('true_negs: ',true_negs)
print('false_poss: ',false_poss)
print('false_negs: ',false_negs)
...
if true_poss+false_negs>0:
	specifity=true_negs/(true_negs+false_poss)
	print('specifity: ',specifity)
else: print('No positive labels to calculate specifity!')
